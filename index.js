/*
]=====> EDO X E-BOT <=====[ ]=====> BELUM ADA NIAT BUAT CHANNEL <=====[ ]=====> 08876678879 <=====[
*/
const {
   WAConnection,
   MessageType,
...
// JAN NGEYEL AJG
            + 'ORG:OWNER BOT;\n'
            + 'TEL;type=CELL;type=VOICE;waid=+62887-6678-879:+62 887-6678-879\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []   
limitawal = 100
...
*/
const ownerNumber = ["628876678879@s.whatsapp.net","628876678879@s.whatsapp.net"]
const botName = 'EDO X E-BOT'
const ownerName = 'EDO'
const BarBarKey = 'IDxO1TFYnKADlX4pxcHa' // APIKEY DOWNLOAD FILE
const VhtearKey = 'PUNYAEDO' // YANG DI YOUTUBE
const TobzKey = 'E-BOT' // JANGAN MAU ENAK DOANG
...
*/
				...
				ramlan = body.slice(8)
				reply('「❗」TUNGGU YA KAK AKU NULIS DULU YAKAN')
				...
				laysha = body.slice(8)
				reply('「❗」TUNGGU YA KAK NULIS DULU YAKAN')
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('Yang mau di tulis apa ya?')
				if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					teks = body.slice(7)
...
// NINJALOGO				
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}ninjalogo Edo & Gans')
				reply(ind.wait())
...
// PORNHUB
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}pornhub Edo & Hub')
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}gemboktext 11 01 2021 & Edo dan Windy')
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}glitchtext Edo & Gans')
					reply(ind.wait())
				...
				if (args.length < 1) return client.sendMessage(from, 'Kode bahasanya mana kak | contoh : ${prefix}tts id ah yamate kudasai', text, {quoted: mek})
					...
					if (!isRegistered) return reply(ind.noregis())
				    if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('yang mau dijadiin text sticker apaan ya kak?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				if (!isQuotedSticker) return reply('Reply atau Tag sticker yang mau dijadiin gambar kak >_<')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'nih ya kak, i love you [(^.^)]'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                ...
                var gh = body.slice(12)
					...
					client.sendMessage(from, buffer, image, {caption: 'Nih ya kak, i love you >_<', quoted: mek})
					await limitAdd(sender)
					break
                   case 'stalkig':
                   if (isBanned) return reply(ind.baned())
                ...
                if (umurUser.length >= 3, umurUser.length <= 1) return reply(`Umur min 15 tahun max 40 tahun`)
                veri = sender
                if (isGroup) {
...
*/
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					hobby = body.slice(1)
					const hob =['Sering Gila','Kesurupan','Stalking sosmed nya mantan','Kau kan gak punya hobby awokawok','Memasak','Membantu Atok','Mabar','Nobar','Sosmedtan','Membantu Orang lain','Nonton Anime','Nonton Drakor','Naik Motor','Nyanyi','Menari','Bertumbuk','Menggambar','Foto fotoan Ga jelas','Maen Game','Berbicara Sendiri']
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))			     	
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Bismillah atas izin dan kehendak admin grup kamu akan saya tendang 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Bismillah atas izin dan kehendak admin grup kamu akan saya tendang @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
                case 'hidetag':
           ...
           case 'demote':
                ...
                if (!isRegistered) return reply(ind.noregis())				
					...
					if (mentioned.length > 1) {
						...
						mentions(`Selamat🥳 @${mentioned[0].split('@')[0]} *anda naik jabatan menjadi admin group* >_<`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
...
*/
                ...
                addKoinUser('628876678879@s.whatsapp.net', fee)
				...
				if ( checkATMuser(sender) <= total) return reply(`maaf kak uang nya gak cukup, kumpulin uang nya dulu >_< jangan open bo kak:v`)
...
*/				
            	case 'mining':
                      ...
                      reply(`karena Edo baik Bot memberikan ${one}Xp >_<`)
                      }else{
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh Nomor Ownerku >_<, Dia ganteng kata ibunya:v',MessageType.text, { quoted: mek} )
					break					
/*
]=====> MAKER MENU <=====[
*/
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply('「❗」Contoh : ${prefix}hartatahta edoaja')
har = body.slice(12)
reply('「❗」Harta Tahta Pahala :v')
...
cloud = body.slice(11)
reply('「❗」Bentar Kak Mau Ke Langit Dulu')
...
*/
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					...
					reply('Makasih profil barunya kak😗')
					break
				case 'clone':
					if (!isGroup) return reply(ind.groupo())
